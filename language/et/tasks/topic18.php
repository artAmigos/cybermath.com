...
$tasks = [
    [
        'question' => 'Milline kuju on kõikide külgede ja nurkadega võrdne?',
        'options' => ['Ristkülik', 'Romblik', 'Ruudu', 'Paralleelogramm'],
        'answer' => 'Ruudu'
    ],
    [
        'question' => 'Kui palju külgi on kuubikul?',
        'options' => [4, 6, 8, 12],
        'answer' => 6
    ],
    [
        'question' => 'Milline kuju ei ole tasane?',
        'options' => ['Ring', 'Kolmnurk', 'Ruudu', 'Pall'],
        'answer' => 'Pall'
    ],
    [
        'question' => 'Kuidas nimetatakse kuju, millel on kolm külge?',
        'options' => ['Ruudu', 'Ristkülik', 'Kolmnurk', 'Romblik'],
        'answer' => 'Kolmnurk'
    ]
];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($score === count($tasks)) {
        // Kontrollime, kas kasutaja on juba selle teema läbinud
        ...
        if ($already_completed == 0) {
            // Arvesta teema läbimine (triger lisab ise 50 münti)
...
<head>
    <meta charset="UTF-8">
    <title>Ülesanded: Geomeetrilised kujundid</title>
...
<body class="container py-5">
    ...
    <?php if ($show_result): ?>
        <div class="alert alert-info animated-alert text-center fs-5">
            Sa lahendasid õigesti <?= $score ?> ülesandest <?= count($tasks) ?>!
            ...
            <div class="mb-4 p-4 task-card">
                ...
                <?php if ($show_result): ?>
                    <?php if (!isset($user_answers[$index])): ?>
                        <div class="mt-2 text-warning">❌ Ei vastatud</div>
